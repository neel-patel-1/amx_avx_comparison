...
from transformers import AutoTokenizer, AutoConfig, AutoModelForCausalLM as HFModel
...
if __name__ == '__main__':
    ...
    # prefer HF token from env (or use huggingface-cli login)
    hf_token = os.environ.get("HF_TOKEN")

    # Load config first (trust_remote_code will execute repo code that registers custom architectures)
    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True, use_auth_token=hf_token)
    # If using a local wrapper that doesn't support trust_remote_code, use HF's AutoModelForCausalLM
